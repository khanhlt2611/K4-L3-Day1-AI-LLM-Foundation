...
def call_openai(
    ...
    """
    try:
        # Import inside the function so tests can safely mock openai.OpenAI.
        ...
        if not response.choices:
            raise ValueError("OpenAI returned a response without choices")
        text = response.choices[0].message.content
        if text is None:
            raise ValueError("OpenAI returned an empty message content")
        return str(text), float(latency)
...
def call_openai_mini(
    ...
    try:
        ...
        raise RuntimeError(f"OpenAI mini request failed: {exc}") from exc
...
def compare_models(prompt: str) -> dict:
    ...
    try:
        ...
        output_tokens_estimate = len(gpt4o_text.split()) / 0.75
        cost = (
        ...
        return {
            ...
            "gpt4o_cost_estimate": float(cost),
        }
    except Exception as exc:
        raise RuntimeError(f"Could not compare models: {exc}") from exc
...
def chat_with_system_prompt(
    ...
    try:
        ...
        raise RuntimeError(f"OpenAI system-prompt request failed: {exc}") from exc
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Deterministic fallback for offline environments and unknown models.
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Could not estimate cost for model '{model}': {exc}") from exc
...
def streaming_chatbot() -> None:
    ...
    try:
        ...
        raise RuntimeError(f"Could not initialize OpenAI client: {exc}") from exc
    ...
    while True:
        try:
            ...
            print(f"Không thể đọc dữ liệu nhập: {exc}")
            break
        ...
        try:
            ...
            print(f"Lỗi khi gọi OpenAI: {exc}")
...
def retry_with_backoff(
    ...
    # The loop always returns or raises. This guard only satisfies static analyzers.
...
def run_assistant(
    ...
    # Avoid initializing an API client when the caller explicitly requests no turns.
    if max_turns == 0:
        return {
            "num_turns": num_turns,
    ...
    while max_turns is None or num_turns < max_turns:
        try:
            ...
            raise RuntimeError(f"Could not read user input: {exc}") from exc

        if not isinstance(user_msg, str):
            raise TypeError("get_input must return a string")
        if user_msg.strip().lower() in ("quit", "exit"):
            break
        ...
        try:
            ...
            # A failed turn is not added to the history or statistics.
            print(f"Lỗi khi xử lý lượt hội thoại: {exc}")
...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    for index, prompt in enumerate(prompts):
        if not isinstance(prompt, str):
            raise TypeError(f"prompts[{index}] must be a string")
        try:
            comparison = compare_models(prompt)
            results.append({"prompt": prompt, **comparison})
        except Exception as exc:
            raise RuntimeError(f"Could not compare prompt at index {index}: {exc}") from exc
    return results
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for index, result in enumerate(results):
        if not isinstance(result, dict):
            raise TypeError(f"results[{index}] must be a dictionary")
        try:
            gpt_latency = float(result.get("gpt4o_latency", 0.0))
            mini_latency = float(result.get("mini_latency", 0.0))
            rows.append(
                [
            ...
            raise ValueError(f"Invalid comparison result at index {index}: {exc}") from exc
